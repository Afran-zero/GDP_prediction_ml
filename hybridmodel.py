...
# 1. Data Loading (same as ML_simple_model.py)
...
# Add lagged targets as features check if its better without lags
lags = [1, 2, 3] 
df_feat = df.copy()
for lag in lags:
...
# 2. Metric Calculation Functions (same as ML_simple_model.py)
...
# 3. Hybrid Model: ARIMA + GBM residual correction (Step-Forward)
def evaluate_hybrid_arima_gbm(df_feat, target_col, feature_cols):
    y = df_feat[target_col].values
    X = df_feat[feature_cols].values
    n = len(y)
    min_train_size = 10
    ...
    for i in range(min_train_size, n):
        ...
        # 2. Get ARIMA in-sample predictions and residuals
        try:
        ...
        # Align X_train with residuals if needed
        ...
        resid_preds.append(resid_pred_gbm)
        ...
        resid_preds.append(resid_pred_rf)
        ...
        # 4. Final prediction = ARIMA forecast + residual correction
        ...
        # Standalone GBM prediction on target (for comparison)
...
results_hybrid = evaluate_hybrid_arima_gbm(df_feat, target_col, feature_cols)
